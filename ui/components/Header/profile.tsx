"use client";
// ProfileDropdown.js
import { useState } from 'react';
import Link from 'next/link';

export default function ProfileDropdown() {
  const [dropdownOpen, setDropdownOpen] = useState(false);

  const toggleDropdown = () => {
    setDropdownOpen(!dropdownOpen);
  };

  return (
    <div className="relative inline-block text-left">
      <div>
        <button
          type="button"
          onClick={toggleDropdown}
          className="flex items-center p-2 bg-gray-800 rounded-full hover:bg-gray-700 focus:outline-none"
        >
          {/* Profile Icon */}
          <img
            className="w-8 h-8 rounded-full"
            src="data:image/png;base64," // Replace with your profile icon image
            alt="Profile"
          />
        </button>
      </div>

      {/* Dropdown menu */}
      {dropdownOpen && (
        <div className="absolute right-0 mt-2 w-48 bg-white shadow-lg rounded-md py-1 ring-1 ring-black ring-opacity-5">
          <Link href="http://localhost:3001"    className="block px-4 py-2 text-gray-700 hover:bg-gray-100">
           Account
          </Link>
          <Link href="/auth/signin" className="block px-4 py-2 text-gray-700 hover:bg-gray-100">
            Sign In
          </Link>
          <Link href="/auth/signup"  className="block px-4 py-2 text-gray-700 hover:bg-gray-100">
            Sign Up
          </Link>
        </div>
      )}
    </div>
  );
}



